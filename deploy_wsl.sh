...
# 复制项目文件到NAS
echo "复制项目文件到NAS..."
sshpass -p "$NAS_PASS" scp -r "$LOCAL_DIR/go.mod" "$LOCAL_DIR/go.sum" "$LOCAL_DIR/cmd" "$LOCAL_DIR/internal" "$LOCAL_DIR/web" "$LOCAL_DIR/data" "$NAS_USER@$NAS_IP:$NAS_DIR/"
...
sshpass -p "$NAS_PASS" scp "$LOCAL_DIR/Dockerfile" "$NAS_USER@$NAS_IP:$NAS_DIR/"

# 创建docker-compose.yml
echo "创建docker-compose.yml..."
cat > "$LOCAL_DIR/docker-compose.yml" << 'EOF'
version: '3'
services:
  orchids-api:
    build: .
...
sshpass -p "$NAS_PASS" scp "$LOCAL_DIR/docker-compose.yml" "$NAS_USER@$NAS_IP:$NAS_DIR/"
...
# 显示部署结果
echo "\n部署完成！"
echo "API地址: http://$NAS_IP:3002"
echo "健康检查: http://$NAS_IP:3002/health"
echo "\n使用以下命令查看日志:"
